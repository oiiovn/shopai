...
try {
    ...
    // Điều kiện đơn giản: chỉ cần status='assigned' và đã quá 30 phút từ assigned_at
    $query_sql = "
        SELECT 
            sub_request_id,
            parent_request_id,
    ...
    if (!$timeout_assigned_query) {
        ...
        echo "Timeout query executed OK\n";
    }
    
    // Fetch all timeout tasks (không check parent campaign nữa)
    $timeout_tasks = [];
    if ($timeout_assigned_query) {
        while ($row = $timeout_assigned_query->fetch_assoc()) {
            echo "  Found timeout task #{$row['sub_request_id']} (parent: #{$row['parent_request_id']}, timeout: {$row['minutes_since_assigned']} min)\n";
            $timeout_tasks[] = $row;
        }
    }
