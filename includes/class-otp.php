...
class OTPRental
{
    ...
    /**
     * Lấy đúng viotp_id từ VIOTP API dựa trên tên dịch vụ
     * Có cache để tránh gọi API nhiều lần
     * 
     * @param string $service_name Tên dịch vụ (đã lowercase)
     * @param string $country
     * @return int|null
     */
    private function getCorrectViotpIdFromAPI($service_name, $country = 'vn')
    {
        static $api_cache = [];
        $cache_key = $country . '_' . md5($service_name);
        
        // Kiểm tra cache (cache trong 1 giờ)
        if (isset($api_cache[$cache_key]) && (time() - $api_cache[$cache_key]['time']) < 3600) {
            return $api_cache[$cache_key]['viotp_id'];
        }
        
        // Chỉ kiểm tra các dịch vụ quan trọng
        $important_services = ['shopee', 'google', 'gmail'];
        $is_important = false;
        foreach ($important_services as $important) {
            if (strpos($service_name, $important) !== false) {
                $is_important = true;
                break;
            }
        }
        ...
        // Lấy từ API
        $api_services = $this->getServicesFromAPI($country);
        if (!isset($api_services['status_code']) || $api_services['status_code'] != 200) {
            return null;
        }
        
        $api_data = $api_services['data'] ?? [];
        $correct_viotp_id = null;
        
        // Tìm đúng dịch vụ
        foreach ($api_data as $api_service) {
            $api_name_lower = strtolower($api_service['name'] ?? '');
            
            if (strpos($service_name, 'shopee') !== false && strpos($api_name_lower, 'shopee') !== false) {
                $correct_viotp_id = $api_service['id'] ?? null;
                break;
            } elseif (strpos($service_name, 'google') !== false && strpos($service_name, 'gmail') === false) {
                // Google (không phải Gmail)
                if (strpos($api_name_lower, 'google') !== false && strpos($api_name_lower, 'gmail') === false) {
                    $correct_viotp_id = $api_service['id'] ?? null;
                    break;
                }
            } elseif (strpos($service_name, 'gmail') !== false) {
                // Gmail
                if (strpos($api_name_lower, 'gmail') !== false) {
                    $correct_viotp_id = $api_service['id'] ?? null;
                    break;
                }
            }
        ...
        // Lưu vào cache
        if ($correct_viotp_id !== null) {
            $api_cache[$cache_key] = [
                'viotp_id' => $correct_viotp_id,
                'time' => time()
            ];
        }
        
        return $correct_viotp_id;
    }
    ...
    public function rentNumber($user_id, $service_id, $network_id = null, $prefix = null, $except_prefix = null)
    {
        try {
            ...
            // Log service_id được nhận
            error_log("OTP Rental - Received service_id: " . $service_id);
            
            // Lấy thông tin dịch vụ - ĐẢM BẢO LẤY ĐÚNG service_id
            $stmt = $this->pdo->prepare("SELECT * FROM otp_services WHERE service_id = ? AND is_active = 1 LIMIT 1");
            $stmt->execute([$service_id]);
            $service = $stmt->fetch();
            
            if (!$service) {
                error_log("OTP Rental - Service not found for service_id: " . $service_id);
                return ['success' => false, 'message' => 'Dịch vụ không tồn tại hoặc đã bị tắt'];
            }
            
            // Log thông tin dịch vụ được lấy từ database
            error_log("OTP Rental - Service found: service_id=" . $service['service_id'] . ", name=" . $service['name'] . ", viotp_id=" . $service['viotp_id']);
            
            // KIỂM TRA VÀ SỬA TỰ ĐỘNG NẾU viotp_id BỊ SAI
            // Lấy đúng viotp_id từ VIOTP API dựa trên tên dịch vụ (với cache để tránh gọi API nhiều lần)
            $service_name_lower = strtolower($service['name']);
            $correct_viotp_id = $this->getCorrectViotpIdFromAPI($service_name_lower, $service['country'] ?? 'vn');
            
            // Nếu tìm thấy viotp_id đúng từ API và khác với database, tự động sửa
            if ($correct_viotp_id !== null && $service['viotp_id'] != $correct_viotp_id) {
                error_log("OTP Rental - WARNING: viotp_id mismatch! service_id={$service_id}, name={$service['name']}, current viotp_id={$service['viotp_id']}, correct viotp_id from API={$correct_viotp_id}");
                
                // Kiểm tra xem có dịch vụ khác đã dùng viotp_id này chưa
                $check_stmt = $this->pdo->prepare("SELECT service_id FROM otp_services WHERE viotp_id = ? AND country = ? AND service_id != ? LIMIT 1");
                $check_stmt->execute([$correct_viotp_id, $service['country'], $service_id]);
                $existing = $check_stmt->fetch();
                
                if ($existing) {
                    // Đã có dịch vụ khác dùng viotp_id này, cần xóa hoặc sửa dịch vụ cũ
                    error_log("OTP Rental - WARNING: viotp_id {$correct_viotp_id} already used by service_id={$existing['service_id']}, updating old service first");
                    // Xóa dịch vụ cũ hoặc set viotp_id = 0
                    $old_stmt = $this->pdo->prepare("UPDATE otp_services SET viotp_id = 0, is_active = 0 WHERE service_id = ?");
                    $old_stmt->execute([$existing['service_id']]);
                }
                
                // Tự động sửa trong database
                try {
                    $fix_stmt = $this->pdo->prepare("UPDATE otp_services SET viotp_id = ? WHERE service_id = ?");
                    $fix_stmt->execute([$correct_viotp_id, $service_id]);
                    // Cập nhật lại service với viotp_id đúng
                    $service['viotp_id'] = $correct_viotp_id;
                    error_log("OTP Rental - Auto-fixed: Updated service_id={$service_id} ({$service['name']}) to viotp_id={$correct_viotp_id} from VIOTP API");
                } catch (PDOException $e) {
                    // Nếu vẫn bị duplicate, log lỗi nhưng không throw
                    error_log("OTP Rental - ERROR: Cannot update viotp_id due to duplicate constraint: " . $e->getMessage());
                    // Vẫn tiếp tục với viotp_id cũ
                }
            }
            ...
            // Log thông tin API params trước khi gọi
            error_log("OTP Rental - API Params before call: serviceId=" . $service['viotp_id'] . " (from service_id=" . $service_id . ", service_name=" . $service['name'] . ")");
