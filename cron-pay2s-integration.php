<?php
/**
 * Cron Job: Tích hợp Pay2S API
 * Lấy giao dịch thật từ Pay2S và xử lý tự động
 */
...
// Log file
$logFile = __DIR__ . '/logs/cron-pay2s.log';
...
function fetchPay2STransactions() {
    try {
        ...
        $url = $config['api_url'] . '/transactions/list';
        ...
        if (!$data['success']) {
            writeLog("Pay2S API Error: " . $data['message']);
            return [];
        }
...
function savePay2STransaction($transaction) {
...
// Bắt đầu cron job
writeLog("=== Bắt đầu cron job tích hợp Pay2S ===");

try {
    // Bước 1: Lấy giao dịch từ Pay2S API
    writeLog("1. Lấy giao dịch từ Pay2S API...");
    $pay2sTransactions = fetchPay2STransactions();
    writeLog("Nhận được " . count($pay2sTransactions) . " giao dịch từ Pay2S");
    ...
    foreach ($pay2sTransactions as $transaction) {
        try {
            ...
            // Lưu giao dịch mới
            savePay2STransaction($transaction);
            $saved++;
            writeLog("✅ Đã lưu giao dịch Pay2S: {$transaction['transaction_id']}");
    ...
    writeLog("Đã lưu $saved/" . count($pay2sTransactions) . " giao dịch Pay2S mới");
    ...
    writeLog("=== Hoàn thành cron job tích hợp Pay2S ===");
