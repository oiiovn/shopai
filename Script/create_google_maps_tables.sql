...
CREATE TABLE IF NOT EXISTS `google_maps_review_sub_requests` (
  ...
  `parent_request_id` int(10) UNSIGNED NOT NULL COMMENT 'ID của chiến dịch mẹ (liên kết đến google_maps_review_requests)',
  ...
  `status` enum('available','assigned','completed','verified','expired') NOT NULL DEFAULT 'available' COMMENT 'Trạng thái: available=chưa gán, assigned=đã gán, completed=đã hoàn thành, verified=đã xác minh, expired=hết hạn',
  ...
  CONSTRAINT `fk_sub_parent` FOREIGN KEY (`parent_request_id`) REFERENCES `google_maps_review_requests`(`request_id`) ON DELETE CASCADE ON UPDATE CASCADE,
